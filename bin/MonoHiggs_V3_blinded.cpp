...
int main(int argc, char** argv) {
  ...
  for (string era : {"13TeV"}) {
    for (string chn : chns) {
      ...
      string file = aux_shapes + year +"/" + input_folders[chn]+"_tmass200.root";
  ...
  cb.cp().channel({"et"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "ZLLjet", "TT" , "otherMC" , "STT", "VVT"}}))
    .AddSyst(cb, "CMS_scale_efaket_1prong1pizero_barrel_"+year, "shape", SystMap<>::init(1.00));
  cb.cp().channel({"et"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "ZLLjet", "TT" , "otherMC" , "STT", "VVT"}}))
    .AddSyst(cb, "CMS_scale_efaket_1prong1pizero_endcap_"+year, "shape", SystMap<>::init(1.00));
  cb.cp().channel({"et"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "ZLLjet", "TT" , "otherMC" , "STT", "VVT"}}))
    .AddSyst(cb, "CMS_scale_efaket_1prong_barrel_"+year, "shape", SystMap<>::init(1.00));
  cb.cp().channel({"et"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "ZLLjet", "TT" , "otherMC" , "STT", "VVT"}}))
    .AddSyst(cb, "CMS_scale_efaket_1prong_endcap_"+year, "shape", SystMap<>::init(1.00));
  cb.cp().channel({"et"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "ZLLjet", "TT" , "otherMC" , "STT", "VVT"}}))
    .AddSyst(cb, "CMS_scale_t_1prong1pizero_"+year, "shape", SystMap<>::init(1.00));
  cb.cp().channel({"et"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "ZLLjet", "TT" , "otherMC" , "STT", "VVT"}}))
    .AddSyst(cb, "CMS_scale_t_1prong_"+year, "shape", SystMap<>::init(1.00));
  cb.cp().channel({"et"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "ZLLjet", "TT" , "otherMC" , "STT", "VVT"}}))
    .AddSyst(cb, "CMS_scale_t_3prong1pizero_"+year, "shape", SystMap<>::init(1.00));
  cb.cp().channel({"et"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "ZLLjet", "TT" , "otherMC" , "STT", "VVT"}}))
    .AddSyst(cb, "CMS_scale_t_3prong_"+year, "shape", SystMap<>::init(1.00));
  cb.cp().channel({"et"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "ZLLjet", "TT" , "otherMC" , "STT", "VVT"}}))
  ...
  cb.cp().channel({"mt"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "ZLLjet", "TT" , "otherMC" , "STT", "VVT"}}))
    .AddSyst(cb, "CMS_scale_mfaket_1prong1pizero_"+year, "shape", SystMap<>::init(1.00));
  cb.cp().channel({"mt"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "TT"}}))
    .AddSyst(cb, "CMS_scale_t_1prong1pizero_"+year, "shape", SystMap<>::init(1.00));
  cb.cp().channel({"mt"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "TT" }}))
    .AddSyst(cb, "CMS_scale_t_1prong_"+year, "shape", SystMap<>::init(1.00));
  cb.cp().channel({"mt"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "TT" }}))
  ...
  cb.cp().channel({"et", "mt", "tt"}).process(ch::JoinStr({sig_procs, {"ZTTjet", "ZLLjet", "TT" , "otherMC" , "STT", "VVT"}}))
  ...
  //! [part7]
  for (string chn : chns) {
    inputFile = input_folders[chn] + "_tmass200.root";
  ...
  string folder = "xtt_datacards_blinded";
  ...
  for (string chn : chns) {
    TFile output((folder + "/common/"+chn+"_"+year+"_2hdma_med"+mediatorMass+"_ps"+pscalarMass+"_tmass200.root").c_str(),
		 "RECREATE");
	...
	cb.cp().channel({chn}).bin({b}).mass({m, "*"}).WriteDatacard(
								     folder + "/" + "2hdma_med"+mediatorMass+"_ps"+pscalarMass + "/" + b + "_" + year+ ".txt", output);
      }
    }
    output.Close();
  }
