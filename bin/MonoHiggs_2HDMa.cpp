...
int main(int argc, char** argv) {
  ...
  config.add_options()
    ...
    ("masspt", po::value<string>(&mass_point)->default_value("2HDMa_gg_sinp_0p35_tanb_1p0_mXd_10_MH3_500_MH4_250"))
  ...
  cout << ">> Making datacards for mass point ="<<mass_point <<"\n";  
  //! [part1]
  ...
  string folder = "xtt_datacards_fullrange_2dhma";
  boost::filesystem::create_directories(folder);
  boost::filesystem::create_directories(folder + "/common");
  boost::filesystem::create_directories(folder + "/" + mass_point);
  for (auto m : masses) {
    boost::filesystem::create_directories(folder + "/" + m);
  }
    
  for (string chn : chns) {
    TFile output((folder + "/common/"+chn+"_"+year+"_"+mass_point+".root").c_str(),
		 "RECREATE");
	...
	cb.cp().channel({chn}).bin({b}).mass({m, "*"}).WriteDatacard(
								     folder + "/" + mass_point + "/" + b + "_" + year+ ".txt", output);
      }
    }
    output.Close();
  }
  string final_folder = folder + "/" + mass_point;
