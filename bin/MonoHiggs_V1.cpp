...
int main(int argc, char** argv) {
  ...
  cb.cp().process({"jetFakes"})
    .AddSyst(cb, "CMS_jetFakesNorm_0jetlow_et_2017", "lnN", SystMap<>::init(1.05));

  cb.cp().process(ch::JoinStr({sig_procs,{"ZLLjet"}}))
    .AddSyst(cb, "CMS_eFakeTau_2017", "lnN", SystMap<>::init(1.15));

  cb.cp().process(ch::JoinStr({sig_procs,{"ZTTjet", "ZLLjet", "TT" , "otherMC" , "STT", "VVT"}}))   ////////////// next add signal here 
  ...
  string folder = "xtt_datacards/"+postfix;
