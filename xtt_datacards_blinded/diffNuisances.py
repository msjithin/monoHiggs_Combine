...
if options.plotfile:
    ...
    histogram.Draw("pe")
    canvas.SaveAs("_pullplot_"+options.plotfile[:-5]+".png")
    fout.WriteTObject(canvas)
    
    width, height = 1280, 720
    if 'cmb' in options.plotfile[:-5]: 
        if '20172018' in options.plotfile[:-5] : width = width*3
        else : width = width*2

    canvas_nuis = ROOT.TCanvas("nuisances", "nuisances", width, height)
    ...
    canvas_nuis.SaveAs("_Nuisance_"+options.plotfile[:-5]+".png")
    ...
    canvas_pferrs = ROOT.TCanvas("post_fit_errs", "post_fit_errs", width, height)
    ...
    hist_fit_e_b.LabelsOption("v")
    hist_fit_e_b.Draw("bar")
    hist_fit_e_s.Draw("barsame")
    leg_rat=ROOT.TLegend(0.6,0.8,0.95,0.95)
    ...
    canvas_pferrs.SaveAs("_Reduction_"+options.plotfile[:-5]+".png")
